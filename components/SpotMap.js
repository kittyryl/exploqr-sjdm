...
function markerIcon(spot, index) {
  const Icon = spotIcon(spot);
  const fill = CATEGORIES[spot.category].fill;
  const svg = renderToStaticMarkup(
    <Icon size={16} color="#ffffff" strokeWidth={2.25} />
  );
  // `name` is usually a string but can be { en, tl }; the aria-label isn't
  // localized here (no locale outside React), so fall back to English.
  const label = (typeof spot.name === "string" ? spot.name : spot.name.en)
    .replace(/"/g, "&quot;");
...
function FitToSpots({ spots, userLocation }) {
  const map = useMap();
  const key =
...
export default function SpotMap({ spots, selectedId, onSelect, userLocation }) {
  ...
  const [mapReady, markReady] = useReducer((n) => n + 1, 0);
  useEffect(() => {
  ...
  const cityBounds = useMemo(
    () => L.latLngBounds(sjdmBoundary.flat()).pad(0.12),
    []
  );
  ...
  return (
    ...
    >
      ...
      <Polygon
        positions={[WORLD_RING, ...sjdmBoundary]}
        interactive={false}
        pathOptions={{
      ...
      {spots.map((spot) => {
        ...
        return (
          <Marker
            key={spot.id}
            ref={(m) => {
              if (m) markerRefs.current.set(spot.id, m);
              else markerRefs.current.delete(spot.id);
            }}
            position={[spot.lat, spot.lng]}
            icon={icons[spot.id]}
            zIndexOffset={selected ? 1000 : 0}
            eventHandlers={{ click: () => onSelect(spot.id), add: markReady }}
          >
            <Tooltip
              direction="top"
