...
const Pano360Viewer = dynamic(() => import("@/components/Pano360Viewer"), {
  ssr: false,
  loading: () => (
...
export default function SpotMedia({ spot }) {
  const { t, text } = useLocale();
  const images = spot.images || [];
  const hasPano = Boolean(spot.pano360);
  const [active, setActive] = useState(hasPano && images.length === 0 ? "pano" : 0);
  ...
  return (
    <figure>
        ...
        {active === "pano" ? (
          ...
          >
            <Image
              ...
              onError={() =>
                setFailed((f) => ({ ...f, [activeImage.src]: true }))
              }
      ...
      {activeImage && !failed[activeImage.src] && (
